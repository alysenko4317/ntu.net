...
namespace EntityLINQ
{
    ...
    class Program
    {
        static void ClearData()  // Очищення даних
        {
            Console.WriteLine("Clean the database: ");

            LearningModelContainer db = new LearningModelContainer();

            // Видалення даних для зв'язку багато-до-багатьох для кожного запису StudentGroup
            // видаляються всі зв'язки з Subject
        ...
        static void InitData()   // Заповнення даних
        {
        ...
        static void Queries()   // Приклади запитів
        {
            ...
            WriteSubjectTypeTree(-1, 0);  // Виведення ієрархії типів предметів
            Console.WriteLine();
            ...
            Console.WriteLine("Отримання всіх курсів і груп, яким вони читаються");
            ...
            Console.WriteLine("\nКількість годин за всіма предметами для кожної групи");
            ...
            Console.WriteLine("\nКількість годин за предметом для всіх груп");
            ...
            Console.WriteLine("\nГрупи, для яких читаються спеціальні курси (з використанням contains)");

            string[] arr = new string[] { "спеціальні", "інший" };
            ...
            Console.WriteLine("\nТипи курсів, що читаються для групи (з повторюваними записами)");
            ...
            Console.WriteLine("\nТипи курсів, що читаються для групи");
            ...
            Console.WriteLine("\nГрупи, яким читаються базові курси");
            ...
            Console.WriteLine("\nГрупи, яким читаються базові курси (використання any)");
            ...
            Console.WriteLine("\nГрупи, яким читаються тільки базові курси (використання all)");
            // у вибірку входитиме лише ИУ5-11, бо групі ИУ5c-11 читають як
            // базові, так і спеціальні курси (умова "all" не виконується, порівняйте з "any"...)
        ...
        public static void IncludeExample()   
        {
            ...
            // NOTE: використовується контекст даних, де LazyLoad не ввімкнений (за замовчуванням вимкнений)
            LearningModelContainer db = new LearningModelContainer();
        ...
        public static void WriteSubjectList(List<Subject> list)   // Виведення списку
        {
        ...
        public static void WriteSubjectTypeTree(int ParentSubjectTypeParam, int LevelParam)
        {
            ...
            {
                // Пошук елементів із заданим елементом верхнього рівня
                q = q.Where(x => x.ParentSubjectType.Id == ParentSubjectTypeParam);
            }

            // Якщо існують елементи на цьому рівні ієрархії
            if (q.Count() > 0)
            {
                // Сортування
                q = q.OrderBy(x => x.TypeName);

                // Перебір усіх значень на заданому рівні ієрархії
                foreach (var x in q)
                {
                    // Вивід відступу
                    if (LevelParam > 0)
                    ...
                    // Рекурсивний виклик функції для всіх елементів, вкладених у поточний
